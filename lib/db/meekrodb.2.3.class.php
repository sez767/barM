...
class DB {
    ...
    public static function setEmptyMDB() {
        self::$mdb = null;
    }
    ...
    public static function queryAssArray() {
        $args = func_get_args();
        return call_user_func_array(array(DB::getMDB(), 'queryAssArray'), $args);
    }

    public static function queryAssData() {
        $args = func_get_args();
        return call_user_func_array(array(DB::getMDB(), 'queryAssData'), $args);
    }
...
class MeekroDB {
    ...
    public function queryAssArray() {
        $args = func_get_args();
        $keyField = array_shift($args);
        $data = $this->prependCall(array($this, 'queryHelper'), $args, 'assoc');
        $ret = array();
        foreach ($data as $value) {
            $ret[$value[$keyField]] = $value;
        }
        return $ret;
    }

    public function queryAssData() {
        $args = func_get_args();
        $keyField = array_shift($args);
        $valueField = array_shift($args);

        $data = $this->prependCall(array($this, 'queryHelper'), $args, 'assoc');
        $ret = array();
        foreach ($data as $value) {
            $ret[$value[$keyField]] = $value[$valueField];
        }
        return $ret;
    }
...
function meekrodb_error_handler($params) {
    ...
    require_once dirname(__FILE__) . '/../Zend/Debug.php';

    $params['REQUEST'] = $_REQUEST;
    $e = new Exception();

    $params['TRACE'] = PHP_EOL . $e->getTraceAsString();

    foreach ($params as $key => $value) {
        error_log(Zend_Debug::dump($value, $key, false));
        ApiLogger::addLogJson(Zend_Debug::dump($value, $key, false));
        if (db_show_errors) {
            Zend_Debug::dump($value, $key, true);
        }
    }
    die();
}
