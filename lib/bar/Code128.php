 ...
 *
 * Modified to make html-output (by Alexandr Gorlov) 10-10-2016
 *
 * Usage example:
 * $c = new Code128;
 * echo $c->draw('Abc123');
...
class Code128 {
	...
	private function getKey($char) {
		switch ($this->type)
		{
			...
			default:
			    throw new Exception("Code128 type is undefined!");
			break;
		}
	}
	/*
	 ...
	 * @return void
	 */
	private function resolveSubtype($data) {
	 ...
	 * @return string
	 */
	private function getStartChar($data) {
		$this->resolveSubtype($data);
		switch($this->type) {
	...
	public function draw($data) {
		...
		// Add the start
		array_unshift($charAry, $this->getStartChar($data));
		// Checksum collector
		$checkSumCollector = $this->getKey($this->getStartChar($data));
	    ...
	    foreach (str_split($res) as $i => $color) {
	      if ($color == 'b') {
	        $html.='<span style="border-left: 1px solid; display: inline-block; height: 20mm;"></span>';
	      } else {
	        $html.='<span style="border-left: white 1px solid; display: inline-block; height: 20mm;"></span>';
	      }
	    }
