...
Ext.define('UserListDemo.view.override.grid', {
    ...
    downloadExcelXml: function (includeHidden, title) {
        ...
        if (!title) {
            title = this.title ? this.title : globalProjectName;
        }
...
//                    console.log(fld);
                ...
                if (cm[j].xtype != 'actioncolumn' && (cm[j].dataIndex != '') && (includeHidden || !cm[j].hidden)) {
                    ...
                    if (cellType[k] !== "None") {
                        ...
                        if (cellType[k] == 'DateTime') {
                            t += Ext.Date.format(v, 'Y-m-d\\TH:i:s.000');
                        } else {
                            if (typeof v == 'string') {
                                t += v.replace(/&/g, '&amp;').replace(/"/g, '&quot;').replace(/'/g, '&#039;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
                            } else {
                                t += v;
                            }
                        }
    ...
    getExportColums: function (includeHidden) {
        var result = {};
        var cm = this.columns;
        var colCount = cm.length;
        // Calculate cell data types and extra class names which affect formatting
        for (var i = 0; i < colCount; i++) {
            if (cm[i].xtype != 'actioncolumn' && (cm[i].dataIndex != '') && (includeHidden || !cm[i].hidden)) {
                var temp = cm[i].dataIndex;
                result[temp] = cm[i].text;
            }
        }
        return result;
    }
