...
if (in_array($_SESSION['Logged_StaffId'], $print_id)) {
    ...
    $fields = "id, fio, price, city_region, offer, dop_tovar, if(country IN ('kzg','am'),addr,addr) as addr, if(country IN ('kzg','am'),phone,'') as phone, kz_curier, status_cur, DATE_FORMAT(date_delivery,'%d-%m'), '' as tt, if(country IN ('kzg','am'),description,''), deliv_desc, other_data, if (country IN ('am'), package, '')";
}
...
if (mysql_num_rows($rs)) {
    ...
    while ($obj = mysql_fetch_assoc($rs)) {
        ...
        $obj['offer'] = $offer; //  . " - " . $obj['package'] . "шт.";
        // additional goods
        ...
        if (
            ...
            foreach ($dop_tovar['dop_tovar'] AS $ke => $va) {
                ...
                $dop_tovar_all[] = $GLOBAL_OFFER_DESC[$dop_tovar['dop_tovar'][$ke]] . " " . implode(" ", $properties); // . " - " . $obj['package'] . "шт.";
            }

            $obj['dop_tovar'] = implode("; ", $dop_tovar_all);
            $obj['offer'] .= "; " . $obj['dop_tovar'];
        }
