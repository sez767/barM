...
if ((int) $_GET['id']) {
    ...
    $query = "SELECT *, offer_date + INTERVAL 3 HOUR as date FROM offers WHERE offer_id = '" . (int) $_GET['id'] . "'";
    if (($offer = DB::queryOneRow($query))) {
        $result = array(
            'success' => true,
            'data' => $offer
        );
...
// collect request parameters
    ...
    if ((int) $_GET['t']) {
        $where .= " AND offers_active ";
    }
    $query = "SELECT *, offer_id as id, offer_date + INTERVAL 3 HOUR as date FROM offers WHERE $where ";
    $total_qury = $query;
    if ($sort != "") {
        $query .= " ORDER BY " . $sort . " " . $dir;
    } else {
        $query .= " ORDER BY offer_accept DESC, offers_active DESC, offer_name ASC";
    }
